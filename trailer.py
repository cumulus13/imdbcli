...
def show(content): 
    ...
    if is_html(content):
        ...
        window = webview.create_window('Custom HTML Viewer', html=content, width=720, height=480, resizable=False, on_top = True)
    else:
        # If content is a URL, create a window with the URL
        window = webview.create_window('IMDb Trailer Browser', content, width=720, height=480, resizable=False, on_top = True)
